...
(For cost visualization and schedule change impact analysis.)

| Field | Type | Notes |
|------|------|-------|
| `cost_capex_usd` | number \| null | Project CAPEX (USD). Projects only. From `capex_$` in `projects_norm.csv`. |
| `cost_opex_annual_usd` | number \| null | Annual OPEX (USD). Projects only. From `annual_opex_$`. |
| `cost_total_p50_usd` | number \| null | Total cost at P50 estimate (USD). Projects only. From `capex_total_p50_$`. |
| `cost_total_p90_usd` | number \| null | Total cost at P90 estimate (USD). Projects only. From `capex_total_p90_$`. |
| `cost_owners_usd` | number \| null | Owner's costs (USD). Projects only. From `owners_costs_$`. |
| `cost_per_day_usd` | number \| null | Cost per day of delay (USD). Calculated as `cost_total_p50_usd / duration_days`. |
| `cost_estimate_usd` | number \| null | Task-level cost estimate (USD). Milestones: rolled up from children. Procurement: from source (null until CSV has cost column). |
| `cost_expedite_premium_usd` | number \| null | Cost premium to expedite (USD). Procurement tasks only. From `expedite_cost_usd` (null until CSV has this column). |
...
### 3.4 `components[]` (Plant Major Components)

Per-plant list of major equipment/components for lifecycle management and degradation modeling. Populated from `plant_major_components.csv`.

| Field | Type | Required | Notes |
|------|------|:--------:|-------|
| `component_id` | string | ✓ | Unique identifier (e.g., `GEN-001`, `XFMR-002`). |
| `component_type` | string | ✓ | Equipment category: `generator`, `turbine`, `transformer`, `boiler`, `condenser`, `reactor_coolant_pump`, `hrsg`, `gas_turbine`, `cooling_tower`, etc. |
| `component_name` | string | ✓ | Human-readable name (e.g., `Main Generator Unit 1`). |
| `install_date` | string (ISO date) |  | Original installation date. |
| `expected_lifespan_years` | number |  | Design life in years. |
| `current_condition_score` | number |  | Current condition (0–100). May differ from plant-level score. |
| `replacement_cost_usd` | number |  | Full replacement cost (USD). |
| `overhaul_cost_usd` | number |  | Major overhaul cost (USD). |
| `degradation_rate_pct_per_year` | number |  | Annual condition degradation without maintenance (%). |
| `deferred_maintenance_penalty_pct` | number |  | Additional degradation per year of deferred maintenance (%). |
| `failure_probability_model` | string |  | Failure curve type: `linear`, `weibull`, `exponential`. |
| `mtbf_hours` | number |  | Mean time between failures (operating hours). |
| `linked_sustainment_id` | string |  | Related sustainment need ID for coordinated planning. |
| `notes` | string |  | Free-form context or assumptions. |

**Computed Fields** (calculated by converter at build time)

| Field | Type | Notes |
|------|------|-------|
| `current_age_years` | number | `today - install_date` in years. |
| `remaining_life_years` | number | `expected_lifespan_years - current_age_years`. |
| `projected_condition_eol` | number | `current_condition_score - (remaining_life_years × degradation_rate_pct_per_year)`. Clamped to ≥0. |
| `replacement_window_start` | string (ISO date) | `install_date + (lifespan - 5) years`. Start of recommended replacement window. |
| `replacement_window_end` | string (ISO date) | `install_date + lifespan years`. End of design life / replacement window. |
